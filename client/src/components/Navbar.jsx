    ...
    relative transition-all ${location.pathname === "/" && "bg-light"} `}>
      ...
      ${open ? "max-sm:translate-x-0" :"max-sm:translate-x-full"}`}>
        ...
        <div className="flex max-sm:flex-col items-start sm:items-center gap-6">
          <button onClick={()=> isOwner ? navigate('/owner') : changeRole()} className='cursor-pointer'>{ isOwner ? 'Dashboard': 'Anuncia tu Vehiculo'}</button>


          <button onClick={()=>{ user ? logout(): setShowLogin(true)}} className='cursor-pointer px-8 py-2 bg-primary hover:bg-primary-dull
          transition-all text-white rounded-lg'>{user ? 'Salir': 'Login'}</button>
        </div>
      </div>
